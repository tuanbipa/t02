...
+ (instancetype)fontWithResourcePath:(NSString *)aPath {
    ...
    dispatch_once(once, ^{ fonts = [[NSMutableDictionary dictionary] retain]; });
...
- (NSDictionary *)glyphMap {
    if (!_glyphMap) @synchronized(self) {
        ...
        if (_glyphMap.count) {
            NSMutableDictionary *glyphMap = [[NSMutableDictionary dictionaryWithContentsOfFile:path] retain];
            ...
            _glyphMap = [[NSMutableDictionary dictionaryWithContentsOfFile:path] retain];
        }
    }
    return _glyphMap;
}
