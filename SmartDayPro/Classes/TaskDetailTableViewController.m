...
- (void)save:(id) sender
{
    ...
    NSDate *sDate = [[(self.task.original != nil?self.task.original.startTime:self.task.startTime) copy] autorelease];

    NSInteger action = (self.task.primaryKey == -1 && self.task.original == nil?TASK_CREATE:TASK_UPDATE);
    
    BOOL isADE = ([self.task isADE] || [self.task.original isADE] || [self.taskCopy isADE]);
    
    BOOL reChange = [self.task isRE] || [self.task.original isRE] || [self.taskCopy isRE];
    
    BOOL reSchedule = NO;
    
    if (self.taskCopy.primaryKey == -1)
    {
        [tm addTask:self.taskCopy];
        
        reSchedule = YES;
    }
    else 
    {
        //BOOL reEdit = [self.task isREInstance] && ![task isREException];
        BOOL reEdit = [self.task isREInstance];
        
        BOOL convertRE2Task = reEdit && [self.taskCopy isTask];
        ...
        {
            reSchedule = [tm updateTask:self.task withTask:self.taskCopy];
            ...
            if (!reSchedule)
            {
                ...
                if (_abstractViewCtrler != nil)
                {
                    ...
                    [[_abstractViewCtrler getSmartListViewController] refreshTaskView4Key:self.taskCopy.primaryKey];
                }
    ...
    */
    ...
    if (_abstractViewCtrler != nil)
    {
        ...
        if (reChange)
        {
            [_abstractViewCtrler.miniMonthView.calView refresh];
        }
        else if ([self.taskCopy isTask])
        {
            if (self.taskCopy.deadline != nil)
            {
                [_abstractViewCtrler.miniMonthView.calView refreshCellByDate:self.taskCopy.deadline];
            }
        }
        else
        {
            [_abstractViewCtrler.miniMonthView.calView refreshCellByDate:self.taskCopy.startTime];
        }
        
        if (isADE)
        {
            [_abstractViewCtrler.miniMonthView.calView refreshADEView];
            [[_abstractViewCtrler getCalendarViewController] refreshADEPane];
        }
        
        [_abstractViewCtrler changeItem:task action:action];
...
- (void) createDeadlineCell:(UITableViewCell *)cell baseTag:(NSInteger)baseTag
{
	...
	firstIconPeriod.selected = (self.taskCopy.deadline != nil && 
                                ...
                                [Common daysBetween:self.taskCopy.deadline andDate:[NSDate date]] == 0);
	...
	secondIconPeriod.selected = (self.taskCopy.deadline != nil && 
                                 ...
                                 [Common daysBetween:self.taskCopy.deadline andDate:[NSDate date]] == 1);
	...
	thirdIconPeriod.selected = (self.taskCopy.deadline != nil && 
                                ...
                                [Common daysBetween:self.taskCopy.deadline andDate:[NSDate date]] == 7);
	...
	fourthIconPeriod.selected = (self.taskCopy.deadline != nil &&
								//[Common compareDateNoTime:self.taskCopy.deadline withDate:[Common dateByAddNumDay:7 toDate:[NSDate date]]] == NSOrderedSame);
                                [Common daysBetween:self.taskCopy.deadline andDate:[NSDate date]] == 30);
...
- (void) createStartCell:(UITableViewCell *)cell baseTag:(NSInteger)baseTag
{
	...
	firstIconPeriod.selected = (self.taskCopy.startTime != nil && 
                                ...
                                [Common daysBetween:self.taskCopy.startTime andDate:[NSDate date]] == 0);
	...
	secondIconPeriod.selected = (self.taskCopy.startTime != nil && 				
                                 ...
                                 [Common daysBetween:self.taskCopy.startTime andDate:[NSDate date]] == 1);
	...
	thirdIconPeriod.selected = (self.taskCopy.startTime != nil && 
                                ...
                                [Common daysBetween:self.taskCopy.startTime andDate:[NSDate date]] == 7);
	...
	fourthIconPeriod.selected = (self.taskCopy.startTime != nil &&
								[Common daysBetween:self.taskCopy.startTime andDate:[NSDate date]] == 30);
