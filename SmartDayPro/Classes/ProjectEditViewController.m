...
- (void)loadView 
{
	...
	mainView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, 320, 480)];
...
- (void) save:(id)sender
{
    ...
    else 
    {
        ...
        //change in SD: does not remove old tag and add new tag when changing categry name
        /*
        if (nameChange)
        {
            NSMutableDictionary *tagDict = [NSMutableDictionary dictionaryWithDictionary:[TagDictionary getTagDict:self.projectCopy.tag]];
            
            [tagDict removeObjectForKey:self.project.name];
            
            [tagDict setObject:self.projectCopy.name forKey:self.projectCopy.name];
            
            self.projectCopy.tag = [TagDictionary createTagByDict:tagDict];
            
            TagDictionary *tagMgr = [TagDictionary getInstance];
            
            [tagMgr deleteTag:self.project.name];
            [tagMgr addTag:self.projectCopy.name];
            
            [tagMgr makePreset:self.projectCopy.name preset:YES];					
            
        }
        
        if (![self.project.tag isEqualToString:self.projectCopy.tag])
        {
            [pm resolveTagChange:self.project tag:self.projectCopy.tag];
        }	
        */
