...
- (void) refreshFrame
{
    ...
    MiniMonthView *miniMonthView = _sdViewCtrler.miniMonthView;
    
    BOOL adeVisible = adeView.adeList != nil && adeView.adeList.count > 0 && miniMonthView.hidden;
    
    CGFloat mmH = miniMonthView.hidden?0:miniMonthView.frame.size.height;
...
-(void)showQuickAdd:(NSDate *)timeSlot
{
    ...
    if ([mm.headerView getMWMode] == 0) //im month mode
    {
    ...
    //CGFloat miniH = _sdViewCtrler.miniMonthView.frame.size.height;
    
    /*
	if (p.y + frm.size.height + miniH > 200)
	{
		CGFloat dy = p.y + frm.size.height + miniH - 200;
