...
-(void) deselect
{
    ...
    */
    ...
    /*if ([self.activeViewCtrler respondsToSelector:@selector(deselect)])
    {
...
- (void) copyCategory
{
	...
	if (plan != nil)
	{
        ...
        /*
		[[ProjectManager getInstance] copyProject:plan];
...
{
	if ([[BusyController getInstance] checkSyncBusy] || view.task.autoSyncPending)
    {
        return;
    }
...
- (void)taskChanged:(NSNotification *)notification
{
	...
	//if (settings.syncSource == 1 && settings.tdAutoSyncEnabled)
    ...
    //else if (settings.syncSource == 0 && settings.sdwAutoSyncEnabled)
...
- (void)eventChanged:(NSNotification *)notification
{
    ...
    //if (settings.syncSource == 0 && settings.sdwAutoSyncEnabled)
...
/*
- (void)taskInitFinished:(NSNotification *)notification
{
	taskBGFinished = YES;
	
	if (taskBGFinished && eventBGFinished)
	{
		[_appDelegate confirmAutoSync];
	}
}

- (void)miniMonthInitFinished:(NSNotification *)notification
{
	eventBGFinished = YES;
