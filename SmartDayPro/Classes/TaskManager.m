...
- (NSMutableArray *) getEventListFromDate:(NSDate *)fromDate toDate:(NSDate *)toDate
{
    ...
    //NSLog(@"get Event List- 6");
	
	[Common sortList:eventList byKey:@"startTime" ascending:YES];
    
    //NSLog(@"get Event List- 7");
...
- (TaskProgress *) getEventSegment:(Task *)task onDate:(NSDate *)onDate
{
	...
	{		
		...
		{
			TaskProgress *ret = [[TaskProgress alloc] autorelease];
...
- (void) addREInstanceToList:(NSMutableArray *)list original:(Task *)re  onDate:(NSDate *)onDate fromDate:(NSDate *)fromDate toDate:(NSDate *) toDate excludeException:(BOOL)excludeException
{
	...
	//if ([fromDate compare:onDate] != NSOrderedDescending && [onDate compare:toDate] == NSOrderedAscending)
...
-(void) findFreeTimeSlotsFromDate:(NSDate *)fromDate inDays:(NSInteger)inDays segments:(NSMutableArray *)segments
{
    ...
    //NSLog(@"find free time - get Event list");
...
{
    //NSLog(@"notify schedule completion");
...
- (BOOL) addTask:(Task *)task
{
	...
	if ([task isTask])
	{
        if (settings.syncEnabled && (settings.sdwAutoSyncEnabled || settings.tdAutoSyncEnabled))
        {
            task.autoSyncPending = YES;
        }
	...
	{
        if (settings.syncEnabled && (settings.sdwAutoSyncEnabled || settings.ekAutoSyncEnabled))
        {
            task.autoSyncPending = YES;
        }
...
- (Task *) getTask2Update:(Task *)taskEdit
{
    ...
    if (taskEdit.listSource != SOURCE_SMARTLIST)
    {
...
-(void) starTask:(Task *)task
{
    ...
    if (task.listSource != SOURCE_SMARTLIST)
    {
        slTask = [self findSmartTask:task];
    }
    */
